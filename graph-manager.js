class GraphManager {
    ...
    processGraphResponse(response) {
        try {
            ...
            if (nodes.length === 0) {
                // Если модель пустая - это нормально, просто показываем сообщение
                this.showMessage('📝 Модель не содержит действий или объектов. Это нормально, если в документе не описаны взаимодействия с системой.', 'info');
                console.log('📝 Модель пустая - документ не содержит описания взаимодействий с системой');
                // Очищаем граф
                if (window.renderGraph) {
                    window.renderGraph({ nodes: [], edges: [] });
                }
