...
# Запускаем API с выводом логов в консоль
echo "🔧 Запуск AI API..."
echo "   Использую api_simple_with_cors.py с CORS поддержкой и выводом JSON"
echo "   📡 Логи API будут отображаться ниже:"
echo "   ========================================"

# Создаем именованный канал для логов API
LOG_PIPE=$(mktemp -u /tmp/api_logs_$$.XXXXXX)
mkfifo "$LOG_PIPE"

# Функция для отображения логов API с префиксом
show_api_logs() {
...
# Запускаем отображение логов в фоновом режиме
show_api_logs &
LOG_DISPLAY_PID=$!
...
# Даем время на запуск и начальную инициализацию
echo "   ⏳ Запуск API... (ожидание 3 секунды)"
sleep 3
