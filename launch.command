...
echo "🔧 Запуск AI API..."
echo "   Использую api_with_forced_logs.py с гарантированным выводом логов"
echo ""
echo "🚀 ЗАПУСК API (логи БУДУТ ВИДНЫ ниже):"
echo "======================================="

# Запускаем API с отключенной буферизацией
python3 -u api_with_forced_logs.py 2>&1 | while read -r line; do
    echo "📢 $line"
    # Принудительно сбрасываем буфер после каждой строки
    /usr/bin/env python3 -c "import sys; sys.stdout.flush()" 2>/dev/null || true
done &
API_PID=$!

# Даем время на запуск
echo "⏳ Запуск API с отключенной буферизацией... (5 секунд)"
sleep 5
