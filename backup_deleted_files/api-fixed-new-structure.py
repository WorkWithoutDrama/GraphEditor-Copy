...
4. Если действия, объекта или его состояния еще нет в модели - добавь их (принцип "если нет - добавь")
...
1. Действия отрисовываются в ПРЯМОУГОЛЬНИКАХ (используют action_id: "a" + 5 цифр)
2. Объект + состояние отрисовывается в ОВАЛЕ (используют составной ID: object_id + state_id)
3. Стрелки соответствуют "connection_out" (источник) → "connection_in" (цель)
4. Правила для связей:
   - Начальные состояния (connection_out) ведут к действию (connection_in)
   - Действия (connection_out) ведут к конечным состояниям (connection_in)
   - Состояния используют составной ID: object_id + state_id (пример: o12345s12345)
   - Действия используют action_id: "a" + 5 цифр
...
class ModelHandler(http.server.BaseHTTPRequestHandler):
    ...
    def handle_generate_model(self):
        ...
        try:
            ...
            # Генерируем модель
            logger.info(f"🔄 ГЕНЕРАЦИЯ МОДЕЛИ через {provider}...")
            ...
            if result.get('success') and 'model' in result:
                ...
                logger.error(f"❌ ОШИБКА ГЕНЕРАЦИИ: {result.get('error', 'Неизвестная ошибка')}")
            ...
            # Логируем отправленный ответ (кратко)
            logger.info(f"📤 ОТПРАВЛЕН ОТВЕТ (успех: {result.get('success', False)})")
