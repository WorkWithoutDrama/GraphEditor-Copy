...
3. Если вернешь старую структуру - ответ будет отклонен
4. Поля action_links и object_links ОБЯЗАТЕЛЬНЫ (могут быть пустыми)
5. connection_in всегда использует составной ID: object_id + state_id (пример: o12345s12345)
...
Верни только JSON."""
    ...
    def _validate_model_structure(self, model):
        ...
        try:
            ...
            # Старая структура имеет ключи как названия действий, а не model_actions
            # Проверяем первые 3 ключа - если любой из них выглядит как старое действие, отвергаем!
            if len(model) > 0:
                first_key = list(model.keys())[0]
                # В старой структуре ключи - названия действий (не model_actions)
                # Новая структура всегда имеет model_actions, model_objects, model_connections
                if 'model_actions' not in model:
                    # Это может быть старая структура
                    # Проверяем, есть ли внутри init_states или final_states
                    for key, value in model.items():
                        if isinstance(value, dict):
                            if 'init_states' in value or 'final_states' in value:
                                ...
                                logger.error(f"   Ожидается новая структура с model_actions, model_objects, model_connections")
                                return False
                    return False
...
class SystemModelHandler(http.server.BaseHTTPRequestHandler):
    ...
    def generate_model_with_llm(self, text):
        ...
        try:
            ...
            # Старый формат - НЕПРИЕМЛЕМ!
            logger.error("❌ LLM вернул старый формат модели")
            logger.error("   Ожидается новая структура с model_actions, model_objects, model_connections")
            # Возвращаем fallback модель в новом формате
            return self.llm_client._get_fallback_model()
